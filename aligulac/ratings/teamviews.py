...
def teams(request):
    ...
    base['teams'] = Group.objects.filter(team=True, active=True)
    ...
    base['inactive'] = Group.objects.filter(team=True, active=False).order_by('name')
...
def team(request, team_id):
    ...
    base['nonplaying'] = GroupMembership.objects.\
                         filter(group=team, current=True, playing=False).order_by('player__tag')
    base['past'] = GroupMembership.objects.\
                   filter(group=team, current=False).order_by('-end', 'player__tag')
